...
bool StitcherPrepared = false;//拼接准备标志位初始化
	...
	const vector<pair<Mat, uchar>> &masks, vector<Mat_<float>> &ec_maps)
{
	...
	ec_maps.resize(num_images);//全局变量ec_maps表示所有块的增益
...
CosiftStitcher::CosiftStitcher(InputArray images)
{
	...
	Prepare(srcVec);//默认使用Classical算法
	if (is_prepared_) {
...
{
	cudaError_t cudaStatus = cudaSetDevice(0);//默认0号GPU
